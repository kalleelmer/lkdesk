...
var cartCtrl = function($filter, $scope, Core, $attrs, cartService, $location) {
	...
	$scope.editShow = function(tickets) {
		...
		Core
			.get("/admin/performances/" + tickets[0].performance_id)
			.then(
				function(response) {
					...
					Core.get(
						"/admin/shows/" + $scope.selectedPerformance.show_id
					...
					Core
						.get(
							"/admin/shows/"
								+ $scope.selectedPerformance.show_id
								+ "/categories")
						.then(
							function(response) {
								...
								for ( var a in $scope.modaldata.categories) {
									Core
										.get(
											"/desk/categories/"
												+ $scope.modaldata.categories[a].id
												+ "/prices")
										.then(
											function(response) {

												$scope.prices = _.concat(
													$scope.prices,
													response.data);
				...
				function(response) {
					alert("Kunde inte hämta föreställningen: "
						+ response.status);
				});
