...
var CustomersCtrl = function($filter, Cart, $scope, $http, User, $routeParams,
	Core, $sce, $location, Notification) {

	$scope.cart = Cart.getCart();
	$scope.profile = User.getUser().profile;

	Core.get("/desk/profiles/" + $scope.profile.id + "/customers/").then(
		function(response) {
	...
	$scope.createCustomer = function(customer) {
		...
		Core.post("/desk/customers/", customer).then(function(response) {
			$scope.customers.push(response.data);
		}, function(error) {
			Notification.error("Fel i skapning av kund: " + error.status)
		})
	};

	$scope.updateCustomer = function() {
		// Core.put("/desk/customers/" + $scope.selectedCustomer.id,
		// $scope.selectedCustomer).then(
		// function(response) {
		//
		// },
		// function(error) {
		// Notifcation.error("Fel i uppdatering av kund: " + error.status)
		// }
		// )
	}
